...
const TeamMemberCard: React.FC<{ member: TeamMember }> = ({ member }) => {
    return (
        <div className="bg-card p-6 rounded-2xl shadow-soft border border-slate-200/50 text-center transform hover:-translate-y-2 transition-transform duration-300">
            <img src={member.avatar_url || `https://avatar.vercel.sh/${member.name}.png?text=${member.name.split(' ').map(n => n[0]).join('')}`} alt={member.name} className="w-24 h-24 rounded-full mx-auto mb-4 object-cover border-4 border-slate-100 shadow-sm" />
